...
@time begin
result = optimize(navier_stokes_residual!, weightss, LevenbergMarquardt())
end

vx = beta*tps_rbf(euclidian_dist_int)* result.minimizer[1:N+M]
vy = tps_rbf(euclidian_dist_int)*result.minimizer[N+M+1:2*(N+M)]

#vx =beta* tps_rbf(euclidian_dist_int)* result[1:N+M]
#vy = tps_rbf(euclidian_dist_int)*result[N+M+1:2*(N+M)]
maximum(vx), maximum(vy)
minimum(vx), minimum(vy)
...
quiver(Xin[:], Yin[:], quiver=(vx, vy), color=:blue, label="Velocity Field")
quiver!(X, Y, quiver=(realvxi, realvyi), color=:red, label="Real Velocity", alpha=0.5)

sum(abs.(vx-realvxi)/realvxi)